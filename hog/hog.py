...
def hefty_hogs(player_score, opponent_score):
    ...
    while score  != 0:
        player_score = digit_fn(score % 10)(player_score)
        score = score // 10
    """
    if opponent_score == 0:
        return 1
...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    "*** YOUR CODE HERE ***"
    if num_rolls == 0:
        temp_1 = hefty_hogs(player_score,opponent_score)
    else:
        temp_1 = roll_dice(num_rolls,dice)
...
def hog_pile(player_score, opponent_score):
    ...
    Digit_Opp = opponent_score % 10
    if Digit_Current == Digit_Opp:
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if score0 > score1:
        message =message = "Player 0 takes the lead by " + str(score0-score1)           
        lead = 0
    elif score0 < score1:
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    while num_roll <= 10:
        temp_2 = make_averaged(roll_dice,total_samples)(num_roll,dice)    
        if num_roll == 1:
            ave = temp_2
            min_roll = 1
        elif temp_2 > ave:
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    score_1 = score + hefty_hogs(score,opponent_score)
    if hog_pile(score_1,opponent_score) != 0:
        return 0
    else:
